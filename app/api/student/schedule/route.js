  ...
  try {
    ...
    if (process.env.BYPASS_AUTH !== "true") {
      const authUser = getCurrentUser(request, {
        requiredRole: ["student", "coordinator"],
      });
      if (authUser.role !== "coordinator" && authUser.userId !== userId) {
        return Response.json(
          { error: "Forbidden. Can only view your own schedule." },
          { status: 403 },
        );
      }
    }
    ...
    const staffMap = Object.fromEntries(
      staffList.map((s) => [s._id.toString(), s]),
    );
    ...
    const sessions = entries
      .map((entry, idx) => {
        ...
        if (
          studentLevel &&
          section &&
          !section.year_levels.includes(studentLevel)
        ) {
          return null;
        }
