...
function generatePDF(student, sessions, days) {
  ...
  const totalCredits = allSessions.filter(s => s.credits > 0).reduce((a, s) => a + s.credits, 0);
...
export default function StudentSchedulePage() {
  ...
  async function fetchSchedule() {
    ...
    setError(null);
    try {
      const res  = await fetch(`/api/student/schedule?userId=${CURRENT_USER_ID}`);
  ...
  function handleDownload() {
    if (!student) return;
    setDownloading(true);
    setTimeout(() => { generatePDF(student, sessions, days); setDownloading(false); }, 400);
  }

  // Uses imported Skeleton and ErrorState components
  if (loading) return <div className="student-schedule-page"><Skeleton /></div>;
  if (error)   return <div className="student-schedule-page"><ErrorState message={error} onRetry={fetchSchedule} /></div>;
    ...
    <div className="student-schedule-page">
      ...
      <section className="hero-section">
        <div className="hero-top-row">
          <div>
            <div className="hero-eyebrow">{student?.semester} · {student?.faculty}</div>
            <h1 className="hero-title">My Schedule</h1>
            <p className="hero-subtitle">
              {student?.name} · {student?.id} · {student?.major} · {student?.level}
            </p>
          </div>
          <div className="hero-actions">
            ...
            {!isPublished && (
              <span className="draft-badge">⚠️ Draft — not published yet</span>
            )}
          </div>
        </div>
      ...
      <section className="stats-row">
        {[
          { icon: "📚", label: "Courses",  value: totalCourses       },
          { icon: "⚡", label: "Sessions", value: allSessions.length },
          { icon: "🎓", label: "Credits",  value: totalCredits       },
          { icon: "📅", label: "Semester", value: student?.semester  },
        ].map(({ icon, label, value }, i) => (
          <div key={label} className="stat-card" style={{ animationDelay: `${100 + i * 60}ms` }}>
            ...
            return (
              ...
              >
                ...
                <div className="week-day-card__sessions">
                  {daySess.map(s => (
                    <div key={s.id} className={`week-session-dot week-session-dot--${s.type.toLowerCase()}`} title={s.title} />
                  ))}
                  {daySess.length === 0 && <span className="week-day-card__free">Free</span>}
                </div>
              </div>
            );
          })}
