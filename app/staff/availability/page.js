...
import WeeklyAvailabilityGrid, {
  slotKey,
} from "../../../components/WeeklyAvailabilityGrid";
...
export default function ProfessorAvailabilityPage() {
  ...
  const [bypassAuthUser, setBypassAuthUser] = useState(null);

  // Get bypass auth user if enabled
  useEffect(() => {
  ...
  // Use bypass auth user ID if available, otherwise use auth user ID
  const currentUserId = bypassAuthUser?.id || auth.userId;
    ...
    <div className="avail-page">
      ...
      <section className="avail-header">
        <div className="avail-header__left">
          <div className="avail-header__eyebrow">
            Spring 2026 | Availability Submission
          </div>
          ...
          {[
            ...
            return (
              <div key={s.id} className="avail-step">
                <div
                  className={`avail-step__circle ${isDone ? "avail-step__circle--done" : ""} ${isActive ? "avail-step__circle--active" : ""}`}
                >
                  {isDone ? (
                    <svg
                      width="14"
                ...
                <span
                  className={`avail-step__label ${isActive ? "avail-step__label--active" : ""}`}
                >
                  {s.label}
                </span>
                {i < arr.length - 1 && (
                  <div
                    className={`avail-step__line ${isDone ? "avail-step__line--done" : ""}`}
                  />
                )}
              </div>
            );
