...
const CURRENT_USER_ID = "69b538e6aa373449d761b13a";
...
export default function ProfessorAvailabilityPage() {
  ...
  useEffect(() => {
    async function loadAvailability() {
      try {
        const res  = await fetch(`/api/staff/availability?userId=${CURRENT_USER_ID}`);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch("/api/staff/availability", {
        ...
        body:    JSON.stringify({ userId: CURRENT_USER_ID, slots }),
      });
