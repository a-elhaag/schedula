...
import { countOverTarget } from "./cost.js";
...
export async function runSolver(institutionId, termLabel) {
  ...
  for (const [level, sessions] of byLevel) {
    ...
    // Phase 3: SA polish — run for longer to really compress campus days
    // Budget scales with number of sessions: ~3ms per session, min 1s, max 4s
    const saBudget = Math.min(4000, Math.max(1000, sessions.length * 3));
    const finalAssignment = assignment.size > 0
      ? anneal(assignment, domains, saBudget)
      : assignment;
    ...
    const unitsOverTarget = countOverTarget(finalAssignment);
    ...
    levelResults.push({
      ...
      stats: {
        ...
        unitsOverTarget,  // how many subgroups attend more than 4 days
      },
    });
  }
