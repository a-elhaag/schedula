 ...
 */
function greedyAssign(sessions, domains) {
 ...
 */
export function solveWithRestarts(sessions, domains, maxRestarts = 3) {
  let bestAssignment = new Map();
  let totalBacktracks = 0;

  for (let r = 0; r < maxRestarts; r++) {
    // Shuffle slightly on restarts for diversity
    const ordered = r === 0
      ? sessions
      : [...sessions].sort(() => Math.random() - 0.5);

    const { assignment, unassigned } = greedyAssign(ordered, domains);

    // Repair phase
    const repaired = repairUnassigned(unassigned, assignment, domains, sessions);
