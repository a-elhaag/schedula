...
export async function getCoordinatorCourses(institutionId, options = {}) {
  ...
  // Fetch enrollments for all courses if termLabel provided
  let enrollmentMap = {};
  if (options.termLabel) {
    const courseIds = courses.map(c => c._id.toString());
    enrollmentMap = await getEnrollmentsByCourseIds(institutionId, options.termLabel, courseIds);
  }
...
export async function getCoordinatorRooms(institutionId, options = {}) {
  ...
  return {
    items: rooms.map((r) => ({
      ...
      lab_type:        r.lab_type ?? null,
      groups_capacity: r.groups_capacity,
...
// NEW: Create room
export async function createRoom(institutionId, roomData) {
  const db = await getDb();
  const institutionObjectId = validateObjectId(institutionId, "institutionId");
  ...
  const room = {
    ...
    lab_type:        roomData.lab_type ?? null,
    groups_capacity: roomData.groups_capacity,
    created_at:      new Date(),
    deleted_at:      null
  };
...
// NEW: Update room
export async function updateRoom(institutionId, roomId, updateData) {
  const db = await getDb();
  const institutionObjectId = validateObjectId(institutionId, "institutionId");
  const roomObjectId = validateObjectId(roomId, "roomId");

  const fieldsToUpdate = {};
  if (updateData.label !== undefined) fieldsToUpdate.label = updateData.label;
  if (updateData.name !== undefined) fieldsToUpdate.name = updateData.name;
  if (updateData.building !== undefined) fieldsToUpdate.building = updateData.building;
  if (updateData.room_type !== undefined) fieldsToUpdate.room_type = updateData.room_type;
  if (updateData.lab_type !== undefined) fieldsToUpdate.lab_type = updateData.lab_type ?? null;
  if (updateData.groups_capacity !== undefined) fieldsToUpdate.groups_capacity = updateData.groups_capacity;

  fieldsToUpdate.updated_at = new Date();

  await db.collection("rooms").updateOne(
    { _id: roomObjectId, institution_id: institutionObjectId },
    { $set: fieldsToUpdate }
  );
...
// NEW: Soft delete room
export async function deleteRoom(institutionId, roomId) {
  const db = await getDb();
  const institutionObjectId = validateObjectId(institutionId, "institutionId");
  const roomObjectId = validateObjectId(roomId, "roomId");
...
export async function updateCourse(institutionId, courseId, updateData) {
  ...
  if (updateData.sections !== undefined) fieldsToUpdate.sections = Number(updateData.sections) || 1;
