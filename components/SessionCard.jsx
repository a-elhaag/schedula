  ...
  return (
    <>
      ...
      <div
        ...
        onClick={() => !closing && setExpanded(true)}
        role="button"
        tabIndex={0}
        onKeyDown={(e) => e.key === "Enter" && !closing && setExpanded(true)}
      >
        ...
        <div className="session-card__main">
          <div className="session-card__header">
            ...
            <span className={`session-badge ${config.bgClass}`}>
              {config.label}
            </span>
          </div>
      ...
      {(expanded || closing) && (
        <div
          className={`session-overlay${closing ? " session-overlay--closing" : ""}`}
          onClick={handleClose}
        >
          <div
            className="session-overlay__card"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="session-overlay__header">
              <div>
                <div className="session-overlay__time">
                  {session.time} – {session.end}
                </div>
