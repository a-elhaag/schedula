...
const validators = {
    ...
    $jsonSchema: {
      ...
      properties: {
        ...
        sections: {
          ...
          items: {
            bsonType: 'object',
            required: ['section_id', 'type', 'year_levels', 'slots_per_week', 'slot_duration_minutes', 'capacity', 'required_room_label', 'assigned_staff'],
            properties: {
  ...
  schedules: {
    $jsonSchema: {
      bsonType: 'object',
      required: ['institution_id', 'term_label', 'approved_by', 'approved_at', 'snapshot_id', 'entries', 'is_published', 'created_at'],
      properties: {
        institution_id: { bsonType: 'objectId' },
        term_label:     { bsonType: 'string' },
...
async function main() {
  ...
  for (const name of collectionNames) {
    const status = await createCollectionSafe(db, name, { $jsonSchema: validators[name].$jsonSchema });
    ...
    console.log(`  ${tag}  ${pad(name, 22)}  (${indexCount} index${indexCount !== 1 ? 'es' : ''})`);
  }
